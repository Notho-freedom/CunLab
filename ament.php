...
<head>
  ...
  <?php if (isset($_GET['delete'])) 
  {
    if ($con->query("delete from tontine where id = '$_GET[delete]'"))
    {
      ...
      $er=mysql_num_rows($con->query("SELECT * FROM rej"));
    if (isset($_GET['rej'])){
      $id= $_GET['rej']+$_SESSION['userId'];
      if (!$con->query("insert into rej (id,idu,idt) values('$id','$_SESSION[userId]','$_GET[rej]')"))
      {
        echo "<div class='alert alert-info text-center'>Vous Avez Déja Rejoint !!</div>";
        
      }else{
      $res=$con->query("SELECT * FROM rej");
      $rowcount=mysqli_num_rows($res);
       if ($rowcount>$er) {
         setNbr($_GET['rej']);
         echo "<div class='alert alert-info text-center'>Vous Avez Rejoint !!</div>";
       }else{
        echo "<div class='alert alert-info text-center'>Vous Avez Déja Rejoint !!</div>";
       }
